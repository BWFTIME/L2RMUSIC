...
class Ashish(Client):
    ...
    async def start(self):
        ...
        # ========== LOGGER_ID Processing (Strictly for Supergroup) ==========
        try:
            ...
            LOGGER(__name__).info(f"Using Logger Group ID: {LOGGER_ID}")
        except (ValueError, TypeError) as e:
            LOGGER(__name__).error(
                f"❌ Invalid LOGGER_ID format: {logger_id_raw!r}. "
                "Please set a valid group integer (e.g., -1002742683802) in config.py or env."
            )
            exit(1)

        # ========== 🛠️ LOAD & RESOLVE LOGGER GROUP FIRST ==========
        try:
            LOGGER(__name__).info("Loading and warming up the logger supergroup...")
            
            # Group ko explicitly cache me laane aur load karne ka tareeqa
            # Pyrogram ko peer database me register karne ke liye get_chat use hota hai
            chat = await self.get_chat(LOGGER_ID)
            
            LOGGER(__name__).info(f"✅ Logger Group successfully loaded: '{chat.title}' (ID: {chat.id})")
            
        except errors.PeerIdInvalid:
            LOGGER(__name__).error(
                f"❌ PeerIdInvalid Error: Bot is not a member of the logger group with ID {LOGGER_ID}.\n"
                "👉 Solution: Make sure the bot is added to that group and promoted as an admin!"
            )
            exit(1)
        except Exception as ex:
            LOGGER(__name__).error(
                f"❌ Failed to load logger group.\n  Reason: {type(ex).__name__} - {ex}"
            )
            exit(1)

        # ========== Send Startup Message to Logger Group ==========
        try:
            await self.send_message(
                chat_id=LOGGER_ID,
                text=f"<u><b>» {self.mention} ʙᴏᴛ sᴛᴀʀᴛᴇᴅ :</b></u>\n\nɪᴅ : <code>{self.id}</code>\nɴᴀᴍᴇ : {self.name}\nᴜsᴇʀɴᴀᴍᴇ : @{self.username}",
            )
            LOGGER(__name__).info("✅ Startup message successfully sent to the logger group.")
        except Exception as ex:
            LOGGER(__name__).warning(f"⚠️ Could not send startup message to logger group: {ex}")

        # ========== Cache Channel/Group Resolution (Optional) ==========
        try:
            ...
            if cache_chat_id != 0:
                ...
                LOGGER(__name__).info(f"✅ Cache chat resolved: {cache_chat_id}")
        except Exception as e:
            LOGGER(__name__).warning(f"⚠️ Cache chat issue (non-critical): {e}")
