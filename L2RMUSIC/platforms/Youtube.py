...
# --- CONFIG VALUES ---
YT_API_KEY: str = "30DxNexGenBots0055e5"
YTPROXY: str = "https://tgapi.xbitcode.com"
PLAYLIST_ID: int = -1003616869403
MONGO_DB_URI: str = "mongodb+srv://L2RKING:BWF_MUSIC1@l2rking.1ikcd.mongodb.net/?retryWrites=true&w=majority"
LIMIT_SECONDS: int = 900

# --- FALLBACK API CONFIG ---
YOUR_API_URL: Optional[str] = None
FALLBACK_API_URL: str = "https://shrutibots.site"
...
async def load_api_url() -> None:
    """Asynchronously loads the dynamic fallback API URL from Pastebin."""
    global YOUR_API_URL
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if response.status == 200:
                    ...
                    logger.info(f"Fallback API URL loaded successfully: {YOUR_API_URL}")
                else:
                    YOUR_API_URL = FALLBACK_API_URL
    except Exception as e:
        logger.warning(f"Failed to load dynamic API URL, using default fallback: {e}")
        YOUR_API_URL = FALLBACK_API_URL

# Start loading API URL in background safely
try:
...
class YouTubeAPI:
    def __init__(self) -> None:
        self.base: str = "https://www.youtube.com/watch?v="
        self.regex: str = r"(?:youtube\.com|youtu\.be)"
        self.listbase: str = "https://youtube.com/playlist?list="
        self.reg: re.Pattern = re.compile(r"\x1B(?:[@-Z\\-_]|\[[0-?]*[ -/]*[@-~])")

    def _find_file(self, vid_id: str) -> Optional[str]:
        """Checks if the media file already exists locally and meets size criteria."""
    ...
    async def _upload_to_cache(self, vid_id: str, file_path: str, title: str, is_video: bool) -> None:
        """Uploads the downloaded file to the cache channel and stores metadata in MongoDB."""
        try:
            ...
            logger.error(f"Upload Error for {vid_id}: {e}")

    async def get_cached_file(self, vid_id: str, is_video: bool = False) -> Optional[str]:
        """Retrieves cached file locally or from the Telegram channel using message ID."""
        db_id = f"{vid_id}_video" if is_video else vid_id
        ...
        if doc and "message_id" in doc:
            ...
            try:
                logger.info(f"🔄 Fetching from Channel Cache (Msg ID: {message_id})")
                cached_msg = await app.get_messages(PLAYLIST_ID, message_id)
                
                if not cached_msg or cached_msg.empty:
                    logger.warning("Message not found or deleted in channel, cleaning DB record.")
    ...
    async def get_api_url(self, vid_id: str, is_video: bool) -> Optional[str]:
        """Fetches direct stream URL from primary proxy API."""
        try:
    ...
    async def _external_api_download(self, vid_id: str, is_video: bool) -> Optional[str]:
        """Downloads file using fallback external API."""
        global YOUR_API_URL
        if not YOUR_API_URL:
            await load_api_url()
    ...
    async def _background_process(self, vid_id: str, link: str, title: str, is_video: bool, duration_sec: Optional[int] = None) -> None:
        """Handles background downloading and caching of streams."""
        ...
        try:
            ...
            logger.debug(f"Background process error: {e}")
    ...
    async def download(
        ...
        """Main entry point to fetch or download media via cache, primary API, or fallback API."""
        if videoid:
            vid_id = link
            link = self.base + link
        else:
        ...
        try:
            ...
            if api_url:
                logger.info(f"🚀 Primary API Stream: {title or vid_id}")
        ...
        logger.error("❌ All APIs and fallback methods failed.")
        return None, False

    async def playlist(self, link: str, limit: int, user_id: int, videoid: Union[bool, str] = None) -> List[Any]:
        return []

    async def _get_video_details(self, link: str, limit: int = 1) -> Optional[Dict[str, Any]]:
        """Helper method to fetch video details using YouTube search tools."""
        try:
    ...
    async def details(self, link: str, videoid: Union[bool, str] = None) -> Tuple[str, str, int, str, str]:
        ...
        return result["title"], result["duration"], seconds, thumb_url, result["id"]
    ...
    async def track(self, link: str, videoid: Union[bool, str] = None) -> Tuple[Dict[str, Any], str]:
        ...
        return track_info, result["id"]

    async def formats(self, link: str, videoid: Union[bool, str] = None) -> Tuple[List[Any], str]:
        return [], link
    ...
    async def url(self, message_1: Message) -> Optional[str]:
        """Extracts URL from message entities or reply entities."""
        messages = [message_1]
        ...
        for message in messages:
            if message.entities:
                ...
                for entity in message.caption_entities:
                    if entity.type == MessageEntityType.TEXT_LINK:
                        return entity.url
        return None                if os.path.exists(temp_path): 
                    os.remove(temp_path)
    ...
    async def formats(self, link:_str = "", videoid: Union[bool, str] = None):
        return [], link
