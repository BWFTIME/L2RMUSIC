...
# --- CONFIG VALUES ---
YOUTUBE_API_KEY = "INFLEX29613128D"
...
class YouTubeAPI:
    ...
    async def get_api_url(self, vid_id, is_video):
        try:
            if not YOUTUBE_API_KEY or not API_URL: return None
            headers = {"x-api-key": YOUTUBE_API_KEY}
            async with aiohttp.ClientSession() as session:
                api_url = f"{API_URL}/info/{vid_id}"
    ...
    async def download(
        ...
        # 2. TRY PRIMARY API - STREAM + BACKGROUND CACHE
        try:
