...
try:
    ...
    # Try to install it automatically (optional)
    try:
...
class YouTubeAPI:
    ...
    async def _ytdlp_download(self, vid_id: str, is_video: bool) -> Optional[str]:
        ...
        ydl_opts = {
            ...
            'audio_format': 'mp3' if not is_video else None,
            'postprocessors': [{
                'key': 'FFmpegExtractAudio' if not is_video else None,
                'preferredcodec': 'mp3',
        ...
        # Remove None postprocessor if not needed
        if not is_video:
            ydl_opts['postprocessors'] = [p for p in ydl_opts['postprocessors'] if p is not None]
        else:
        ...
        try:
            ...
            def download_sync():
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    ...
                    # The actual file path is constructed from the output template
                    # Since we used outtmpl with %(ext)s, we need to find the file
                    # We'll check for the vid_id with possible extensions
                    for ext_check in ["mp4", "m4a", "mp3", "webm"]:
                        path = f"downloads/{vid_id}.{ext_check}"
                        if os.path.exists(path) and os.path.getsize(path) > 2048:
                            return path
                    # If not found, try to get from info
