...
PLAYLIST_ID = -1003616869403   # verify this ID
...
async def check_cache_channel():
    ...
    try:
        ...
        logger.warning(f"⚠️ Cache channel inaccessible: {e} – cache disabled.")
...
class YouTubeAPI:
    ...
    # ---- Search cache by title (case-insensitive, partial match) ----
    async def _search_cache_by_title(self, query: str) -> Optional[dict]:
        """Search trackdb for a cached song matching the query (by title)."""
        try:
            # Use regex for case-insensitive partial match
            regex = re.compile(query, re.IGNORECASE)
            doc = await trackdb.find_one({"title": {"$regex": regex}})
            if doc:
                # doc contains vid_id, title, type (audio/video)
                vid_id = doc["vid_id"]
                # If vid_id ends with "_video", strip it for consistency
                if vid_id.endswith("_video"):
                    vid_id = vid_id[:-6]
                return {"vid_id": vid_id, "title": doc["title"], "is_video": doc.get("type") == "video"}
        except Exception as e:
