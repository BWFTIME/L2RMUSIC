...
PLAYLIST_ID = -1003616869403   # verify this ID is correct
...
# --- Global cache channel status ---
_CACHE_AVAILABLE = None

async def check_cache_channel():
    """Check if the bot can access the cache channel. Returns True/False."""
    global _CACHE_AVAILABLE
    if _CACHE_AVAILABLE is not None:
        return _CACHE_AVAILABLE
    try:
        chat = await app.get_chat(PLAYLIST_ID)
        # If we can get the chat, the bot is a member.
        _CACHE_AVAILABLE = True
        logger.info(f"✅ Cache channel accessible: {chat.title} (ID: {PLAYLIST_ID})")
        return True
    except (PeerIdInvalid, ChatAdminRequired, Exception) as e:
        logger.warning(f"⚠️ Cache channel inaccessible: {e} – cache will be disabled for this session.")
...
try:
    ...
    if loop.is_running():
        ...
        asyncio.create_task(check_cache_channel())
    else:
        loop.run_until_complete(load_api_url())
        loop.run_until_complete(check_cache_channel())
except RuntimeError:
    pass
...
class YouTubeAPI:
    ...
    async def get_cached_file(self, vid_id: str, is_video: bool = False) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Peer ID invalid for channel {PLAYLIST_ID}. The bot may not be a member. "
                         f"Cleaning DB entry for {vid_id} and disabling cache for this session.")
            await trackdb.delete_one({"vid_id": db_id})
            # Disable cache globally to avoid repeated errors
            global _CACHE_AVAILABLE
            _CACHE_AVAILABLE = False
            return None
        except Exception as e:
            ...
            if message.entities:
                ...
                for entity in message.caption_entities:
                    if entity.type == MessageEntityType.TEXT_LINK:
                        return entity.url
        return None                    {"$set": {"message_id": msg.id, "title": title, "type": "video" if is_video else "audio"}},
                    upsert=True
                )
                logger.info(f"✅ Upload complete (msg_id={msg.id}): {title}")
                return True
